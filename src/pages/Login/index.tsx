...
export default function Login({ setIsAuthenticated }) {
  ...
  const handleLogin = () => {
    ...
    // Login simples fictício — você pode substituir por integração com backend
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="flex flex-col bg-white shadow-2xl rounded-s-4xl p-8  w-full max-w-md">
        ...
        <input ref={inputEmail} type="text" id="email" className="w-full p-2 border border-gray-300 rounded-md placeholder-gray-500" />

        <label htmlFor="pass" className="text-md mb-2 mt-4">Senha</label>
        <input ref={inputPass} type="password" id="pass" className="w-full p-2 border border-gray-300 rounded-md placeholder-gray-500" />
        ...
        <button
          onClick={handleLogin}
          className="w-full bg-black text-white p-3 rounded-lg mt-6 hover:bg-gray-800"
        >
          Fazer Login
        </button>
      ...
      <div className="relative">
        <img
          src={`https://img.freepik.com/vetores-premium/conjunto-de-icones-de-estilo-plano-de-cinema-e-filme_185107-63.jpg`}
          alt=""
          className="w-[400px] h-[533px] hidden rounded-r-4xl md:block object-cover"
        />
        <div className="absolute hidden bottom-10 right-3 p-6 bg-white opacity-40 rounded md:block">
          <div className="text-black text-xl">Seus filmes de uma maneira gratuita!</div>
        </div>
      </div>
    </div>
  );
